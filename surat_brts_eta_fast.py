...
def scrape_single_stop_worker(stop_id):
    """
    Worker used by the concurrent all-stops scraper.
    Each worker gets its own Session because requests.Session
    is not intended to be shared concurrently.
    """
...
def scrape_all_brt_eta(max_workers=8):
    """
    Scrape all BRT stops concurrently.

    max_workers controls the number of simultaneous stop sessions.
    Start around 5-10; avoid very high values because the target
    server is an external government service.
    """
    # Get the stop list only once.
    session = make_session()
    ...
    # IMPORTANT: only pass stop IDs to workers.
    # Each worker creates its own requests.Session().
    with ThreadPoolExecutor(
        ...
        for future in as_completed(future_to_stop):
            ...
            try:
                ...
                print(
                    f"[{completed}/{len(stops)}] "
                    f"{result.get('stop_name', stop['name'])} "
                    f"({stop['id']}) "
    ...
    # Restore original stop ordering because concurrent workers finish
    # in arbitrary order.
    stop_order = {
        stop["id"]: index
    ...
    results.sort(
        key=lambda item: stop_order.get(
            item["stop_id"],
            999999,
        )
    )
